                ...
                for (const key in jsonData) {
                    if (jsonData.hasOwnProperty(key)) {
                        ...
                        div.innerHTML = `
                            ...
                            <button onclick="b4d.extension.load(['${list.join(`','`)}'], '${key}', '${item.Color}', '${item.Category}', '${item.Dependencies}', '${item.customXML.replaceAll('"','\"')}')" class="market-button" id="${key}">Add to Toolbox</button>
                        `;
